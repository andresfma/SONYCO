...
export default function Categorias() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await api.get('/exportar/categorias', {
      ...
      let filename = 'categorias_export.xlsx'; // nombre por defecto
      ...
      console.error('Error al exportar categorias:', err);
  ...
  const handleCategoriaDetail = (categoria: Categoria) => {
    // Navegar a página de detalle del categoria
    navigate(`/categorias/${categoria.id}`);
  };

  const handleCategoriaEdit = (categoria: Categoria) => {
    // Navegar a página de edición del categoria
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-8 pb-2">
        ...
        <div>
          ...
          <p className="text-cl_font_sec mt-1">
            Crea, edita y administra cada detalle de tus categorías.
          </p>
        </div>
      ...
      <CategoriasTable
        onCategoriaDetail={handleCategoriaDetail}
        onCategoriaEdit={handleCategoriaEdit}
        showActions={{
          detail: true,
          edit: true,
          toggleActive: true,
