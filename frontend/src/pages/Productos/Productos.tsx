...
export default function Productos() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await api.get('/exportar/productos', {
      ...
      let filename = 'productos_export.xlsx'; // nombre por defecto
  ...
  const handleProductDetail = (producto: Producto) => {
    // Navegar a página de detalle del producto
    navigate(`/productos/${producto.id}`);
  };

  const handleProductEdit = (producto: Producto) => {
    // Navegar a página de edición del producto
      ...
      <div className="flex justify-between items-center mb-8 pb-2">
        ...
        <div>
          ...
          <p className="text-cl_font_sec mt-1">
            Crea, edita y administra cada detalle de tus productos.
          </p>
        </div>
