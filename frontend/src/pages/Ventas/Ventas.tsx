...
export default function Ventas() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await api.get('/exportar/ventas', {
      ...
      let filename = 'ventas_export.xlsx'; // nombre por defecto
  ...
  const handleVentaDetail = (venta: Venta) => {
    // Navegar a página de detalle del venta
    navigate(`/ventas/${venta.id}`);
  };

  const handleVentaEdit = (venta: Venta) => {
    // Navegar a página de edición del venta
  ...
  return (
    < div className="p-4" >
      ...
      <div className="flex justify-between items-center mb-8 pb-2">
        ...
        <div>
          ...
          <p className="text-cl_font_sec mt-1">
            Crea, edita y administra cada detalle de tus ventas.
          </p>
        </div>
      ...
      <VentasTable
        onVentaDetail={handleVentaDetail}
        onVentaEdit={handleVentaEdit}
        showActions={{
          detail: true,
          edit: true,
          toggleActive: true,
