...
export default function Clientes() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await api.get('/exportar/clientes', {
      ...
      let filename = 'clientes_export.xlsx'; // nombre por defecto
  ...
  const handleClientDetail = (cliente: Cliente) => {
    // Navegar a página de detalle del cliente
    navigate(`/clientes/${cliente.id}`);
  };

  const handleClientEdit = (cliente: Cliente) => {
    // Navegar a página de edición del cliente
      ...
      <div className="flex justify-between items-center mb-8 pb-2">
        ...
        <div>
          ...
          <p className="text-cl_font_sec mt-1">
            Crea, edita y administra cada detalle de tus clientes.
          </p>
        </div>
